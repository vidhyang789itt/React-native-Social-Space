...
export const fetchMyProfilePosts = createAsyncThunk(
  "posts/fetchMyProfilePosts",
  async (userId: string, { rejectWithValue }) => {
    try {
      const data: any = await getUserPostsApi(userId);
      return data.body;
...
export const fetchViewedUserPosts = createAsyncThunk(
  "posts/fetchViewedUserPosts",
  async (userId: string, { rejectWithValue }) => {
    try {
      const data: any = await getUserPostsApi(userId);
      return data.body;
...
const postSlice = createSlice({
  ...
  reducers: {
    ...
    clearMyProfilePosts: (state) => {
      state.myProfilePosts = [];
    },
    clearViewedUserPosts: (state) => {
      state.viewedUserPosts = [];
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMyProfilePosts.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchMyProfilePosts.fulfilled, (state, action) => {
        state.loading = false;
        state.myProfilePosts = action.payload;
      })
      .addCase(fetchMyProfilePosts.rejected, (state, action: any) => {
        state.loading = false;
        state.error = action.payload;
      })

      .addCase(fetchViewedUserPosts.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchViewedUserPosts.fulfilled, (state, action) => {
        state.loading = false;
        state.viewedUserPosts = action.payload;
      })
      .addCase(fetchViewedUserPosts.rejected, (state, action: any) => {
        state.loading = false;
        state.error = action.payload;
      });
