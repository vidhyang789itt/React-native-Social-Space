...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    handleMessageDelivered: (state, action) => {
      const { messageId, deliveredBy, deliveredAt } = action.payload;

      const msg = state.activeMessages.find((m) => m._id === messageId);
      if (!msg) return;

      const alreadyDelivered = msg.deliveredTo?.some((item: any) => {
        const userId =
          typeof item.userId === "string" ? item.userId : item.userId?.userId;
        return userId === deliveredBy;
      });
    ...
    handleMessagesRead: (state, action) => {
      const { conversationId, readBy, readAt } = action.payload;

      state.activeMessages = state.activeMessages.map((msg) => {
        if (msg.conversationId !== conversationId) return msg;

        const senderUserId = msg.senderId?.userId;
        if (senderUserId === readBy) return msg;

        const alreadyRead = msg.readBy?.some((item: any) => {
          const userId =
            typeof item.userId === "string" ? item.userId : item.userId?.userId;
          return userId === readBy;
        });
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(startConversation.rejected, (state, action) => {
        console.error("❌ Start conversation failed:", action.payload);
      })
      ...
      .addCase(createGroupChat.rejected, (state, action) => {
        console.error("❌ Create group chat failed:", action.payload);
      })
      ...
      .addCase(updateGroupImage.rejected, (state, action) => {
        console.error("❌ Update group image failed:", action.payload);
      })
      ...
      .addCase(deleteMsgForMe.pending, (state) => {
        console.log("⏳ deleteMsgForMe: API call in progress...");
      })
      ...
      .addCase(deleteMsgForMe.rejected, (state, action) => {
        console.error("❌ deleteMsgForMe: API Failed", action.payload);
      });
  },
});
